...
private:
    ...
    struct Direct_Mapping_Structure
    {
        // Constructor
        Direct_Mapping_Structure(DirectMapped & d) :
        blockSize(d.blockSize), wordSize(d.wordSize),addressEvicted(""),
        address(d.addressList[std::rand() % d.addressList.size()]),
        wordQuantity(d.wordQuantity),
        instructionMap(getInstructionMap(address, d.addressMap)),
        wordCharacters(getWordCharacters(d.wordQuantity)),
        addressHashCode(GenerateHashCode(this -> address)),
        tag(address.substr(0, address.size() - d.indexSize - d.offsetSize)),
        indexHashCode(GenerateHashCode(setIndex)),
        offset(address.substr(address.size() - d.offsetSize, d.offsetSize)),
        setIndex(address.substr(d.mainMemorySize - (d.indexSize + d.offsetSize),d.indexSize))
        {
